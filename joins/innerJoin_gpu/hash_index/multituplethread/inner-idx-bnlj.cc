...
void join(){
  ...
  */
  ...
  block_x = left/TH_TUPLE < BLOCK_SIZE_X ? left/TH_TUPLE : BLOCK_SIZE_X;
  grid_x = left / (TH_TUPLE*block_x);
  if (left % (block_x*TH_TUPLE) != 0)
    grid_x++;
  ...
  /*count */
  res = cuMemAlloc(&c_dev, (block_x*grid_x+1) * sizeof(uint));
  if (res != CUDA_SUCCESS) {
  ...
  *******************************************************************/
  ...
  if(!(presum(&c_dev,(uint)block_x*grid_x+1))){
  ...
  ************************************************************************/
  ...
  if(!transport(c_dev,(uint)block_x*grid_x+1,&jt_size)){
