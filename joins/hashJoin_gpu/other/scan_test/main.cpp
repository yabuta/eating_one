...
int main(int argc, char **argv)
{
    ...
    for (uint arrayLength = MIN_LL_SIZE; arrayLength <= MAX_LL_SIZE; arrayLength <<= 1)
    {
      ...
      for (int i = 0; i < iCycles; i++)
        {
          szWorkgroup = scanExclusiveLL(d_Output, d_Input, N2 / arrayLength, arrayLength);
        }
      ...
      printf("Validating the results...\n");
      printf("...reading back GPU results\n");
      checkCudaErrors(cudaMemcpy(h_OutputGPU, d_Output, N2 * sizeof(uint), cudaMemcpyDeviceToHost));
      
      printf("...scanExclusiveHost()\n");
      sdkResetTimer(&hTimer);
      sdkStartTimer(&hTimer);
      
      scanExclusiveHost(h_OutputCPU, h_Input, N2 / arrayLength, arrayLength);
      ...
      // Data log
      if (true)//(arrayLength == MAX_LL_SIZE)
        {
          printf("\n");
          printf("scan-Large, Throughput = %.4f MElements/s, Time = %.5f s, Size = %u Elements, NumDevsUsed = %u, Workgroup = %u\n",
                 (1.0e-6 * (double)arrayLength/timerValue), timerValue, (unsigned int)arrayLength, 1, (unsigned int)szWorkgroup);
          printf("\n");
          printf("CPU Time = %.5f s\n",CPUtimerValue);
          printf("GPU = %d\tCPU = %d\n",h_OutputGPU[N2-1],h_OutputCPU[N2-1]);
