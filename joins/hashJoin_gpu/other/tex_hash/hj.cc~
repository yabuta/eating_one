...
join()
{
  ...
  CUdevice dev;
  CUcontext ctx;
  CUfunction function,c_function,cp_function_lt,cp_function_rt,p_function_lt,p_function_rt;
  ...
  struct timeval temp_s,temp_f,tex_time_s,tex_time_f;
  double time_cal;
  ...
  res = cuModuleGetFunction(&p_function_lt, p_module, "partitioning_lt");
  if (res != CUDA_SUCCESS) {
    printf("cuModuleGetFunction(partitioning) failed\n");
    exit(1);
  }

  res = cuModuleGetFunction(&p_function_rt, p_module, "partitioning_rt");
  if (res != CUDA_SUCCESS) {
    printf("cuModuleGetFunction(partitioning) failed\n");
    exit(1);
  }

  res = cuModuleGetFunction(&cp_function_lt, p_module, "count_partitioning_lt");
  if (res != CUDA_SUCCESS) {
    printf("cuModuleGetFunction(count_partitioning) failed\n");
    exit(1);
  }

  res = cuModuleGetFunction(&cp_function_rt, p_module, "count_partitioning_rt");
  if (res != CUDA_SUCCESS) {
    printf("cuModuleGetFunction(count_partitioning) failed\n");
    exit(1);
  }
  ...
  ***************************************************************/
  ...
  if (t_num % p_block_x != 0)
    p_grid_x++;
  /*
  p_block_x = t_num / p_grid_x;
  if (t_num % p_grid_x != 0)
    p_block_x++;
  */

  gettimeofday(&tex_time_s, NULL);

  /*
  int *tex_lt;
  tex_lt = (int *)calloc(left,sizeof(int));  
  for(int i=0;i<left;i++){
    tex_lt[i] = lt[i].val;
  }
  */
  ...
  res = cuModuleGetTexRef(&lt_texref, p_module, "lt_tex");
  if(res != CUDA_SUCCESS) {
    printf("cuModuleGetTexRef(lt) failed: res = %d\n -> %s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }

  res = cuModuleGetTexRef(&rt_texref, p_module, "rt_tex");
  if(res != CUDA_SUCCESS) {
  ...
  gettimeofday(&tex_time_f, NULL);
  temper = (tex_time_f.tv_sec - tex_time_s.tv_sec) * 1000 * 1000 + (tex_time_f.tv_usec - tex_time_s.tv_usec);

  printf("tex init: %ld us (%ld ms)\n", temper, temper/1000);
  temper = 0;
  ...
  if(res != CUDA_SUCCESS) {
    printf("1cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  ***********************************/
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("2cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  ***************************************************************/
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("3cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("4cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  *******************************************************************/
  ...
  // get handles to a texture memory 
  CUtexref plt_texref, prt_texref;
  res = cuModuleGetTexRef(&plt_texref, module, "plt_tex");
  if(res != CUDA_SUCCESS) {
    printf("cuModuleGetTexRef(plt) failed: res = %d\n -> %s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }

  res = cuModuleGetTexRef(&prt_texref, module, "prt_tex");
  if(res != CUDA_SUCCESS) {
    printf("cuModuleGetTexRef(prt) failed: res = %d\n -> %s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }


  // bind texture memorys on GPU 
  res = cuTexRefSetAddress(NULL, plt_texref, plt_dev, left * sizeof(TUPLE));
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetAddress(plt_dev) failed: res = %d\n -> %s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }

  res = cuTexRefSetAddress(NULL, prt_texref, prt_dev, right * sizeof(TUPLE));
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetAddress(prt_dev) failed: res = %d\n -> %s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }


  // texture memory configuration 
  res = cuTexRefSetFlags(plt_texref, CU_TRSF_READ_AS_INTEGER);
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetFlags(plt_texref) failed: res = %d\n->%s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }

  res = cuTexRefSetFlags(prt_texref, CU_TRSF_READ_AS_INTEGER);
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetFlags(prt_texref) failed: res = %d\n->%s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }  

  res = cuTexRefSetFormat(plt_texref, CU_AD_FORMAT_SIGNED_INT32, 2);
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetFormat(plt_texref) failed: res = %d\n->%s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }

  res = cuTexRefSetFormat(prt_texref, CU_AD_FORMAT_SIGNED_INT32, 2);
  if(res != CUDA_SUCCESS) {
    printf("cuTexRefSetFormat(prt_texref) failed: res = %d\n->%s\n", res, getCudaDrvErrorString(res));
    exit(1);
  }
