  ...
  */
    /*    for (int offset = 1; offset < size; offset <<= 1)
    {
      if(pos>offset){
        ...
        __syncthreads();
        uint t = x + __shfl(x,pos-offset,threadIdx.x);
        __syncthreads();
        x = t;
      }
...
extern "C" const uint MAX_BATCH_ELEMENTS = 4 * SHORT_SIZE * THREADBLOCK_SIZE * THREADBLOCK_SIZE;//must be < 4 * THREADBLOCK_SIZE * THREADBLOCK_SIZE * THREADBLOCK_SIZE
extern "C" const uint MIN_SHORT_ARRAY_SIZE = 4;
extern "C" const uint MAX_SHORT_ARRAY_SIZE = 4 * SHORT_SIZE;
extern "C" const uint MIN_LARGE_ARRAY_SIZE = 8 * SHORT_SIZE;
extern "C" const uint MAX_LARGE_ARRAY_SIZE = 4 * SHORT_SIZE * THREADBLOCK_SIZE;
extern "C" const uint MIN_LL_SIZE = 8 * SHORT_SIZE * THREADBLOCK_SIZE;
...
)
{
    ...
    assert(arrayLength % (4 * SHORT_SIZE) == 0);

    scanExclusiveShared<<<arrayLength / (4 * SHORT_SIZE), THREADBLOCK_SIZE>>>(
        (uint4 *)d_Dst,
        (uint4 *)d_Src,
        arrayLength
    );
...
)
{
    ...
    scanExclusiveShared<<<arrayLength / (4 * SHORT_SIZE), SHORT_SIZE>>>(
        (uint4 *)d_Dst,
        (uint4 *)d_Src,
        4 * SHORT_SIZE
    );
    ...
    scanExclusiveShared2<<< blockCount2, THREADBLOCK_SIZE>>>(
        ...
        (uint *)d_Dst,
        (uint *)d_Src,
        arrayLength / (4 * SHORT_SIZE),
        arrayLength / (4 * SHORT_SIZE)
    );
    getLastCudaError("scanExclusiveShared2() execution FAILED\n");

    uniformUpdate<<<arrayLength / (4 * SHORT_SIZE), SHORT_SIZE>>>(
        (uint4 *)d_Dst,
        (uint *)d_Buf
    );
...
)
{
  ...
  */
  ...
  for(uint i = 2; i<=THREADBLOCK_SIZE ; i <<= 1){
    if(i >= arrayLength/(4 * SHORT_SIZE * THREADBLOCK_SIZE)){
      array_temp = i;
    }
  }
  ...
  scanExclusiveShared3<<< blockCount3, THREADBLOCK_SIZE>>>(
        ...
        (uint *)d_Dst,
        (uint *)d_Src,
        arrayLength / (4 * SHORT_SIZE * THREADBLOCK_SIZE),
        array_temp
  ...
  uniformUpdate2<<<arrayLength / (4 * SHORT_SIZE ), SHORT_SIZE>>>(
        (uint4 *)d_Dst,
        (uint *)e_Buf
    );
  getLastCudaError("uniformUpdate() execution FAILED\n");
