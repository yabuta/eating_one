...
int main(int argc, char **argv)
{
    ...
    const uint N = 64 * 1048576 ;//13 * 1048576 / 2;
    ...
    checkCudaErrors(cudaHostAlloc((void **)&d_Input,N*sizeof(uint),cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc((void **)&d_Output,N*sizeof(uint),cudaHostAllocDefault));
    ...
    */
    ...
    for (int i = 0; i < iCycles; i++)
      {
        szWorkgroup = scanExclusiveLL(d_Output, d_Input, N);
      }
    ...
    printf("\n");
    printf("scan-Large, Throughput = %.4f MElements/s, Time = %.5f s, Size = %u Elements, NumDevsUsed = %u, Workgroup = %u\n",
           (1.0e-6 * (double)N/timerValue), timerValue, (unsigned int)N, 1, (unsigned int)szWorkgroup);
    printf("\n");
    printf("CPU Time = %.5f s\n",CPUtimerValue);
    printf("GPU = %d\tCPU = %d\n",h_OutputGPU[N-1],h_OutputCPU[N-1]);
    

    printf("Shutting down...\n");
    closeScan();
    checkCudaErrors(cudaFreeHost(d_Output));
    checkCudaErrors(cudaFreeHost(d_Input));
