...
join()
{
  ...
  struct timeval temp_s,temp_f,time_init_s,time_init_f;
  double time_cal;
  ...
  ***********************************/
  ...
  p_grid_x = p_grid_x * p_num/ONETH_PART_NUM;
  ...
  *******************************************************************/
  ...
  count = (uint *)calloc(right,sizeof(uint));

  gettimeofday(&time_init_s, NULL);

  checkCudaErrors(cudaMemset((void *)count_dev, 0 , grid_x*block_x*grid_y*sizeof(uint)));

  gettimeofday(&time_init_f, NULL);
  ...
  uint jt_size;
  res = cuMemcpyDtoH(&jt_size,jt_size_dev,sizeof(uint)); 
  if(res != CUDA_SUCCESS){
  ...
  */
  ...
  printDiff(time_init_s,time_init_f);
