...
void createTuple()
{
  ...
  for (int i = 0; i < right; i++) {
    ...
    rt[i].val = i;//rand()%SELECTIVITY; // selectivity in tuple.h
  ...
  for (int i = 0; i < left; i++) {
    ...
    if(i < RES_MAX){
      lt[i].val = i;//rand()%SELECTIVITY; // selectivity in tuple.h
    }else{
      lt[i].val = i + right;//rand()%SELECTIVITY; // selectivity in tuple.h
    }
