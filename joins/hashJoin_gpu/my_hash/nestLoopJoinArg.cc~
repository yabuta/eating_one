...
init(void)
{
  ...
  for (int i = 0; i < arg_right; i++) {
    ...
    for(int j = 0;j<VAL_NUM;j++){
      Tright[i].val[j] = rand()%SELECTIVITY; // selectivity for jt size = 1000000 
    }
  ...
  for (int i = 0; i < arg_left; i++) {
    ...
    for(int j = 0; j < VAL_NUM;j++){
      Tleft[i].val[j] = rand()%SELECTIVITY; // selectivity for jt size = 1000000 
    }
...
join()
{
  ...
  */
  ...
  time_cal = (time_send_f.tv_sec - time_send_s.tv_sec)*1000*1000 + (time_send_f.tv_usec - time_send_s.tv_usec);
  printf("Calculation send with Devise    : %6f(micro sec)\n", time_cal);
