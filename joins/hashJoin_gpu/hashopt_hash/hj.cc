...
join()
{
  ...
  ***************************************************************/
  ...
  for(uint tp=PARTITION ; tp<=p_num ; tp=tp*PARTITION){
    ...
    /********************************************************************/
    /*
    uint *ll = (uint *)calloc(t_num*p_n,sizeof(uint));
    res = cuMemcpyDtoH(ll,lL_dev,t_num*p_n * sizeof(uint)); 
    if(res != CUDA_SUCCESS){
      printf("cuMemcpyDtoH (lt) failed: res = %lu\n", (unsigned long)res);
      exit(1);    
    }
    for(uint j=0; j<t_num*p_n; j++){
      printf("lL[%d] = %d\n",j,ll[j]);
    }
    */
  ...
  */
  ...
  res = cuMemcpyDtoH(lt,plt_dev,left * sizeof(TUPLE)); 
  if(res != CUDA_SUCCESS){
    printf("cuMemcpyDtoH (lt) failed: res = %lu\n", (unsigned long)res);
    exit(1);    
  }
  ...
  p_sum = (uint *)calloc(p_n,sizeof(uint));

  if(!(p_sum_dev = diff_part(lL_dev,t_num,p_n,left))){
    printf("p_sum prefix sum error.\n");
    exit(1);
  }
  res = cuMemcpyDtoH(p_sum,p_sum_dev,p_n * sizeof(uint)); 
  if(res != CUDA_SUCCESS){
