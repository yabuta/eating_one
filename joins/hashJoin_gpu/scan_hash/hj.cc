...
join()
{
  ...
  int *count,*lL,*rL,*p_sum,*lL_temp;
  CUresult res;
  CUdevice dev;
  CUcontext ctx;
  CUfunction function,c_function,cp_function,p_function;
  CUmodule module,c_module,p_module,cp_module;
  CUdeviceptr lt_dev, rt_dev, jt_dev,count_dev,lL_dev,rL_dev,lL_temp_dev;
  ...
  ***************************************************************/
  ...
  if(res != CUDA_SUCCESS) {
    printf("lhash count cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  //thrust::exclusive_scan(lL,lL + t_num*p_num,lL);

  lL_temp = (int *)calloc(p_num * t_num,sizeof(int));

  for(int i=0; i < p_num*t_num ; i++){
    printf("lL[%d] = %d\n",i,lL_temp[i]);
  }

  res = cuMemAlloc(&lL_temp_dev, p_num * t_num * sizeof(int));
  if (res != CUDA_SUCCESS) {
    printf("cuMemAlloc (lL) failed\n");
    exit(1);
  }

  res = cuMemcpyHtoD(lL_temp_dev, lL_temp, p_num * t_num * sizeof(int));
  if (res != CUDA_SUCCESS) {
    printf("cuMemcpyHtoD (lL_temp) failed: res = %lu\n", (unsigned long)res);
    exit(1);
  }
  ...
  //calculate min number larger than t_num*p_num
  int temp_scan_num = t_num*p_num;
  int batch_size = 0;

  if(temp_scan_num%MAX_LARGE_ARRAY_SIZE == 0){
    batch_size = temp_scan_num / MAX_LARGE_ARRAY_SIZE;
  }else{
    batch_size = temp_scan_num / MAX_LARGE_ARRAY_SIZE + 1;
  }

  scanExclusiveLarge((uint *)lL_dev, (uint *)lL_dev, batch_size , MAX_LARGE_ARRAY_SIZE);
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("lhash presum cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  ***********************************/
  ...
  if(res != CUDA_SUCCESS) {
    printf("lhash partition cuLaunchKernel() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }      
  ...
  ***************************************************************/
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("rhash count cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  //thrust::exclusive_scan(rL,rL + t_num*p_num,rL);
  ...
  if(res != CUDA_SUCCESS) {
    printf("rhash partition cuLaunchKernel() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }      

  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("rhash partition cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  *******************************************************************/
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("count cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
  ...
  ************************************************************************/
  ...
  res = cuCtxSynchronize();
  if(res != CUDA_SUCCESS) {
    printf("join cuCtxSynchronize() failed: res = %lu\n", (unsigned long int)res);
    exit(1);
  }  
