...
void createTuple()
{
  ...
  /*
  res = cuMemHostAlloc((void**)&rt,rsize * sizeof(TUPLE),CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
    printf("cuMemHostAlloc to RIGHT_TUPLE failed: res = %lu\n", (unsigned long)res);
    exit(1);
  }
  res = cuMemHostAlloc((void**)&lt,lsize * sizeof(TUPLE),CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
...
uint hashJoin(
              ...
              CUfunction lcp_function,
              CUfunction lp_function,
              CUfunction sp_function,
              CUfunction rcp_function,
              CUfunction rp_function
)
{
  ...
  CUdeviceptr plt_dev,prt_dev,l_p_dev,radix_dev;
  CUdeviceptr lstartPos_dev,rstartPos_dev;
  ...
  ************************************************************************/
  ...
  res = cuMemFree(lstartPos_dev);
  if (res != CUDA_SUCCESS) {
    printf("cuMemFree (lstartPos_dev) failed: res = %lu\n", (unsigned long)res);
    exit(1);
  }
...
join()
{
  ...
  while(1){
    ...
    while(1){
      ...
      jt_size = hashJoin(function,c_function,lcp_function,lp_function,sp_function,rcp_function,rp_function);
  ...
  printf("Diff: %ld us (%ld ms)\n", rightread_time, rightread_time/1000);
