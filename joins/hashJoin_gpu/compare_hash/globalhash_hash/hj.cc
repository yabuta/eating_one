...
join()
{
  ...
  ************************************************/
  ...
  for(uint i=0 ; i<p_num ;i++ ){
    ...
    if(p_sum[i]%SHARED_SIZE == 0 && p_sum[i]!=0){
      l_p_num += p_sum[i]/SHARED_SIZE;
    }else{
      l_p_num += p_sum[i]/SHARED_SIZE + 1;
    }            
  }
  ...
  for(int i=0; i<p_num; i++){
    if(p_sum[i]/SHARED_SIZE < 1 || p_sum[i]==SHARED_SIZE){
      l_p[l_p_num] = p_loc[i];
      radix_num[l_p_num] = i;
      l_p_num++;
    }else{
      if(p_sum[i]%SHARED_SIZE == 0){
        temp2 = p_sum[i]/SHARED_SIZE;
      }else{
        temp2 = p_sum[i]/SHARED_SIZE + 1;
      }
      l_p[l_p_num] = p_loc[i];
      radix_num[l_p_num] = i;
      l_p_num++;
      for(uint j=1 ; j<temp2 ; j++){
        l_p[l_p_num] = p_loc[i]+j*SHARED_SIZE;
        radix_num[l_p_num] = i;
        l_p_num++;
      }
    }
