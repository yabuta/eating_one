...
void createTuple()
{
  ...
  for (int i = 0; i < right; i++) {
    ...
    for(int j = 0;j<NUM_VAL;j++){
      rt[i].val = rand()%100; // selectivity = 0.01
    }
  ...
  for (int i = 0; i < left; i++) {
    ...
    for(int j = 0; j < NUM_VAL;j++){
      lt[i].val = rand()%100; // selectivity = 0.01
    }
  ...
  res = cuMemHostAlloc((void**)&jt, right * left * sizeof(RESULT),CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
...
createIndex(void)
{
  ...
  for (int i = 0; i < left; i++) {
    if (!(pidx->nxt = (IDX *)calloc(1, sizeof(IDX)))) ERR;
    pidx = pidx->nxt;
    pidx->val = lt[i].val;
    pidx->adr = i;
  }
