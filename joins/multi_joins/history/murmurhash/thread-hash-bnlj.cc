...
unsigned int MurmurHash2 (unsigned int key,unsigned int seed )
{
...
executor(void *a)
{
  ...
  while(true){
    ...
    HASHTABLE *HashTable;
    HashTable = (HASHTABLE *)malloc(tBuf.rsize*sizeof(HASHTABLE));

    
    for(int i=0 ; i<tBuf.rsize ; i++){
      int hashval = MurmurHash2(rtemp[i].val,0);

      for(int j=0; j<tBuf.rsize ; j++){
        if(HashTable[j].haveval == false){
          HashTable[j].hash == hashval;
          HASHCHILD *newchild;
          newchild = (HASHCHILD *)malloc(sizeof(HASHCHILD));
          HashTable[j]->head = newchild;
          newchild->val = rtemp[i].val;
          newchild->adr = i;
          break;
        }else if(HashTable[j].hash == hashval){
          HASHCHILD *temp=HashTable[j].head;          
          HASHCHILD *newchild;
    ...
    for (int i = 0; i < tBuf.lsize ; i++){
      int hashval = MurmurHash2(ltemp[i].val,0);
      for (int j=0 ; j<tBuf.rsize ; j++){
        if(HashTable[j].hash == hashval){
          HASHCHILD *temp = HashTable[j].head;
          while(1){
            if(temp->val == ltemp[i].val){
              if (pthread_mutex_lock(&Lk)) ERR;
              jt[resultVal].rkey = ltemp[i].key;
              jt[resultVal].rval = ltemp[i].val;
              jt[resultVal].skey = rtemp[temp->adr].key;
              jt[resultVal].sval = rtemp[temp->adr].val;
              resultVal++;
              if (pthread_mutex_unlock(&Lk)) ERR;
            }
    ...
    for(int i=0 ; i<tBuf.rsize ; i++){
      if(HashTable[i].haveval == true){
        HASHCHILD *temp = HashTable[i].head;
        while(1){
          if(temp->next != NULL){
            HASHCHILD *temp2 = temp->next;
            free(temp);
            temp = temp2;
          }else{
            free(temp);
