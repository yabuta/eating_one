...
void *count(void *a){
  ...
  int startPos,endPos;
  if(PARTITION%THREAD_NUM != 0){
    startPos = threadidx*(PARTITION/THREAD_NUM+1);
  }else{
    startPos = threadidx*(PARTITION/THREAD_NUM);
  }

  if(PARTITION%THREAD_NUM != 0){
    endPos = (threadidx+1)*(PARTITION/THREAD_NUM+1);
  }else{
    endPos = (threadidx+1)*(PARTITION/THREAD_NUM);
  }

  
  for(int part = startPos; part<endPos; part++){
    ...
    if(part == 0){
      ltemp = plt;
      lsize = LLS[THREAD_NUM*(part+1)-1];
      rtemp = prt;
      rsize = RLS[THREAD_NUM*(part+1)-1];
    }else{
      ltemp = &(plt[LLS[THREAD_NUM*part-1]]);
      lsize = LLS[THREAD_NUM*(part+1)-1] - LLS[THREAD_NUM*part-1];
      rtemp = &(prt[RLS[THREAD_NUM*part-1]]);
      rsize = RLS[THREAD_NUM*(part+1)-1] - RLS[THREAD_NUM*part-1];
    }
...
executor(void *a)
{
  ...
  int position = counter[threadidx];
  ...
  for(int part = startPos; part<endPos; part++){
    ...
    for (int i = 0; i < lsize ; i++){
       for (int j=0 ; j < rsize ; j++){
         ...
         if (rtemp[j].val == ltemp[i].val){
           if (pthread_mutex_lock(&Lk)) ERR;
           jt[position].rkey = rtemp[j].key;
           jt[position].rval = rtemp[j].val;
           jt[position].skey = ltemp[i].key;
           jt[position].sval = ltemp[i].val;
           position++;
           if (pthread_mutex_unlock(&Lk)) ERR;
         }
       }
...
createThreads(void)
{
  ...
  */
  ...
  for(int i = 0; i<THREAD_NUM; i++){
    if (pthread_create(&(thEx[i]), NULL, count, &(thid[i]))) ERR;
