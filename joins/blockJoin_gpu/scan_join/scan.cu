...
{
  d_Data[0] = d_Src[loc-1]+d_Src[loc2-1];
}
...
//transport input data to output on loc
...
//add input data on loc and loc2
...
)
{
  ...
  const uint blockCount = 1;//iDivUp(arrayLength , LOOP_PERTHREAD2*THREADBLOCK_SIZE);
  add_kernel<<<blockCount, 1>>>(
                                      d_Dst,
                                      d_Src,
                                      loc,
                                      loc2
                                      );
  getLastCudaError("add_gpu() execution FAILED\n");
  checkCudaErrors(cudaDeviceSynchronize());
