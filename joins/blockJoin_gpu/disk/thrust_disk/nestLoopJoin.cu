...
uint nonIndexJoin(
  ...
  uint total = 0;
  CUresult res;
  CUdeviceptr lt_dev, rt_dev, jt_dev,count_dev;
  ...
  for(uint ll = 0; ll < arg_left ; ll += PART){
    ...
    count the number of match tuple
      ...
      */
      int *test_data = reinterpret_cast<int *>(count_dev);
      thrust::device_ptr<int> thrust_test = thrust::device_pointer_cast(test_data);

      thrust::exclusive_scan(thrust_test,
                             thrust_test + gpu_size,
                             thrust_test);
